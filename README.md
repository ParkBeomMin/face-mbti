...
### 📱 서버 없이 폰에서 전부 관리하기 (추천!)

위 0️⃣~2️⃣ 과정을 **GitHub Actions가 대신** 해줘요. PC도, 서버도, 티처블 머신 사이트도 필요 없어요.

1. 폰 브라우저(또는 GitHub 앱)에서 `tools/celebs.csv` 를 열어 ✏️ 연예인 목록 수정 → 커밋
2. 저장소 **Actions 탭 → "MBTI 모델 수집·학습" → Run workflow** 버튼
   - 옵션은 기본값 그대로도 OK (인물당 장수, 특정 유형만, 학습 횟수 조절 가능)
3. 15~30분 뒤 완료되면 학습된 모델이 `models/tm/` 에 **자동 커밋**
4. `main` 브랜치에서 돌렸다면 GitHub Pages가 재배포 → 앱 새로고침하면 새 모델 적용! 🎉

수집된 사진을 검수하고 싶으면 Run 옵션에서 `upload_dataset` 을 켜세요 —
사진 zip이 아티팩트(3일 보관)로 올라와서 폰에서도 내려받아 확인할 수 있어요.
(사진은 저장소에 커밋되지 않고, 학습이 끝나면 러너와 함께 사라져요)
